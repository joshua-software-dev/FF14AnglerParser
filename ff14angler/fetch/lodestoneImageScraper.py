...
from ff14angler.constants.values import ANGLER_API_BASE_URL, ANGLER_PAGE_LOAD_WAIT_DURATION, EXPORT_DIRECTORY
...
class LodestoneImageScraper:
    ...
    @classmethod
    async def get_large_icon_and_url(cls, driver: WebDriver, short_icon_url: str, lodestone_url: str) -> str:
        ...
        large_icon_path: str = os.path.join(
            EXPORT_DIRECTORY,
            # (BASE_URL)/i/006000/010110l.png -> i/006000/010110l.png
            large_icon_url.replace(ANGLER_API_BASE_URL, '').lstrip('/')
        )
        ...
        if not os.path.isfile(large_icon_path):
            with open(large_icon_path, 'wb+') as fh:
                fh.write(await cls.fetch_large_image_from_lodestone_url(driver, lodestone_url))
