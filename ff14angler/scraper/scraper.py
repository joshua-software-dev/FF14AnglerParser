...
class Scraper:
    ...
    async def main(driver: WebDriver):
        ...
        await BaitScraper.ensure_all_bait_mooch_fish_up_to_date()
        await SpotScraper.collect_spot_data(driver)

        print('Writing JSON dump form scraping results...')
        os.makedirs(config_settings['EXPORT_DIRECTORY'], exist_ok=True)
        with open(os.path.join(config_settings['EXPORT_DIRECTORY'], 'full_dump.json'), 'w+') as fh:
